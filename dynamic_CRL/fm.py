...
from particles import GraphSpace, canonical_dimension
...
class FMMetadata:
    ...
    def build(cls, cfg: Any, graph_space: GraphSpace) -> "FMMetadata":
        canon = canonical_dimension(cfg)
        h_child_dim = canon + 3
        child_scale = np.r_[
            np.full(canon, cfg.H_COEF_SCALE),
            cfg.H_LOGVAR_SCALE,
            cfg.H_INT_MEAN_SCALE,
            cfg.H_LOGVAR_SCALE,
        ].astype(np.float32)
        return cls(
...
def graph_to_lag_labels(
    ...
    return np.array(
        [int(graph[parent, child] > 0.5) for parent, child in graph_space.lag_edge_list],
        dtype=np.int64,
    )
...
def lag_labels_to_graph(
    ...
    for state, (parent, child) in zip(labels, graph_space.lag_edge_list):
...
def graph_to_inst_labels(
    ...
    for left, right in graph_space.inst_pair_list:
        labels.append(
            1 if graph[left, right] > 0.5 else (2 if graph[right, left] > 0.5 else 0)
        )
...
def inst_labels_to_graph(
    ...
    graph = np.zeros((cfg.A, cfg.A), np.float32)
    for state, (left, right) in zip(labels, graph_space.inst_pair_list):
        state = int(state)
        if state == 1:
...
def pack_H_raw(mechanisms: list[dict]) -> np.ndarray:
    ...
    for mechanism in mechanisms:
        output.extend(mechanism["beta_full"].tolist())
        output.extend(
            [
                np.log(mechanism["obs_var"]),
                mechanism["int_mean"],
                np.log(mechanism["int_var"]),
            ]
        )
...
def H_target_mask(
    ...
    for child in range(cfg.A):
        mask = np.zeros(metadata.h_child_dim, np.float32)
        mask[0:2] = 1.0
        for parent in range(cfg.A):
            if lag_graph[parent, child] > 0.5:
                mask[2 + parent] = 1.0
            if inst_graph[parent, child] > 0.5:
                mask[2 + cfg.A + parent] = 1.0
        mask[metadata.canon :] = 1.0
        masks.append(mask)
        ...
        for child in order:
            ...
            if intervention[child] > 0.5:
                current[child] = rng.normal(
                    mechanism["int_mean"],
                    np.sqrt(max(mechanism["int_var"], 1e-8)),
                )
            else:
                beta = mechanism["beta_full"]
                mean = beta[0] + beta[1] * previous[child]
                for parent in range(cfg.A):
                    mean += (
                        beta[2 + parent]
                ...
                current[child] = rng.normal(
                    mean, np.sqrt(max(mechanism["obs_var"], 1e-8))
                )
...
def latent_to_observation(
    ...
    with torch.no_grad():
        observation = decoder(
            torch.tensor(latent_raw, dtype=torch.float32, device=data.device)
        ).cpu().numpy()
    decoder.train()
...
def make_fm_simulation_batch(
    ...
    for component in range(len(pair_indices)):
        lag_graph, inst_graph = graph_space.graph_pairs[int(pair_indices[component])]
        for intervention in data.true_model.intervention_conditions:
            for _ in range(cfg.FM_SIM_REPS_PER_COMPONENT_INTERVENTION):
                ...
                lag_targets.append(graph_to_lag_labels(lag_graph, graph_space))
                inst_targets.append(graph_to_inst_labels(inst_graph, graph_space))
                h_targets.append(pack_H_scaled(mechanisms[component], metadata))
                h_masks.append(H_target_mask(lag_graph, inst_graph, cfg, metadata))
    ...
    return {
        ...
        "h": torch.tensor(
            np.stack(h_targets), dtype=torch.float32, device=data.device
        ),
        "h_mask": torch.tensor(
            np.stack(h_masks), dtype=torch.float32, device=data.device
        ),
        "weight": torch.tensor(
            np.asarray(weights), dtype=torch.float32, device=data.device
        ),
        "lengths": np.asarray([len(recording) for recording in recordings]),
    }
...
def fm_update_from_batch(
    ...
    for _ in range(cfg.FM_UPDATES_PER_OUTER):
        ...
        indices = torch.randint(0, total_batch, (batch_size,), device=device)
        ...
        time = cfg.FM_GRAPH_EPS + (1 - 2 * cfg.FM_GRAPH_EPS) * torch.rand(
            batch_size, 1, device=device
        )
        keep_lag = torch.rand(batch_size, metadata.e_lag, device=device) < time
        keep_inst = torch.rand(batch_size, metadata.e_inst, device=device) < time
...
def prepare_context_for_test(
    ...
    recordings = [observations[index] for index in range(len(observations))]
...
def defog_absorbing_rates(
    ...
    derivative_current = derivative.gather(-1, current_index).squeeze(-1)
    support_size = (p_time > 0).sum(-1).float()

    denominator = support_size * p_current
    rates = torch.relu(derivative - derivative_current[..., None]) / denominator[..., None]
    if eta != 0:
...
def sample_H_given_fixed_graph(
    ...
    with torch.no_grad():
        for _ in range(n_samples):
            h_state = torch.randn(metadata.h_dim, dtype=torch.float32, device=device)
