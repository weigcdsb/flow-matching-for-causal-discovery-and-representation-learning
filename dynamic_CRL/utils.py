...
def set_seed(cfg: Any) -> torch.device:
    ...
    if device.type == "cuda":
        ...
        # Strict deterministic mode can fail for CUDA attention/CuBLAS kernels.
...
def normalized_time_encoding(valid_mask: torch.Tensor, width: int) -> torch.Tensor:
    ...
    positions = torch.arange(max_length, device=valid_mask.device)[None, :].expand(batch_size, -1)
    ...
    if encoding.shape[-1] < width:
        encoding = torch.nn.functional.pad(encoding, (0, width - encoding.shape[-1]))
...
def pad_y_list(
    ...
    padded = np.zeros((len(recordings), max_length, observation_dim), np.float32)
