...
class RecordingContext(nn.Module):
    ...
    def forward(
        ...
        intervention_repeated = intervention[:, None, :].expand(
            -1, max_length, -1
        )
...
class CausalEncoder(nn.Module):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        return self.base(
            observations
        ) + self.cfg.ENCODER_RESIDUAL_STRENGTH * self.residual(observations)


class ResidualDecoder(nn.Module):
    def __init__(self, cfg: Any, encoder: CausalEncoder, data: Any):
        super().__init__()
